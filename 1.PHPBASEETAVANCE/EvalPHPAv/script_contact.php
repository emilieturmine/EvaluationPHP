...
if (empty($_POST["Nom"])) {
    header("Location: form_contact.php?erreur= Nom");
    exit;
...
if (empty($_POST["Prénom"])) {
    header("Location: form_contact.php?erreur=Prénom");
    exit;
...
if (empty($_POST["Sexe"])) {
    header("Location: form_contact.php?erreur=Sexe");
    exit;
    echo " Le sexe doit être renseigné.";
    $erreur_count++;
};
if (empty($_POST["DateNaissance"])) {
    header("Location: form_contact.php?erreur= DateNaissance");
    exit;
    echo "La date de naissance doit être renseignée.";
    $erreur_count++;
};
if (empty($_POST["cp"])) {
    header("Location: form_contact.php?erreur=cp");
    exit;
...
if (empty($_POST["Ville"])) {
    header("Location:form_contact.php?erreur=Ville");
    exit;
    echo " La ville doit être renseignée.";
    $erreur_count++;
};
if (empty($_POST["Email"])) {
    header("Location: form_contact.php?erreur=Email");
    exit;
...
if (empty($_POST["Sujet"])) {
    header("Location: form_contact.php?erreur=Sujet");
    exit;
    echo " Le Sujet doit être séléctionné.";
    $erreur_count++;
};
if (empty($_POST["Question"])) {
    header("Location: form_contact.php?erreur=Question");
    exit;
...
if (empty($_POST["Accord"])) {
    header("Location: form_contact.php?erreur=Accord");
    exit;
